...
description: Draft and, after explicit user approval, create concise, clearly marked GitHub issues and comments for concrete problems discovered outside active work. Use for unrelated defects, regressions, missing features, documentation gaps, or test gaps, and when the user asks to file or comment on an issue.
---
...
6. Before any issue, comment, or label write, show the user the exact destination and proposed title, body or comment, and labels. Ask for explicit approval and wait for it; the original request to report the finding is not approval of the final draft.
7. After approval, publish exactly the approved draft. If the destination or content changes materially, show the revised draft and obtain approval again. Do not ask for another confirmation when nothing material changed.
8. Verify the resulting URL and, for a new issue, the label before reporting success. If access, authentication, or permissions prevent the write, report the blocker and do not claim that it happened.
